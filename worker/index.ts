...
// --- Static Assets ---
// Cache-Control headers are set via public/_headers file (Cloudflare static assets
// bypass the Worker, so headers must be configured declaratively).

/** Serve static assets from ASSETS binding (fallback for non-matched routes) */
app.get("*", async (c) => {
  try {
